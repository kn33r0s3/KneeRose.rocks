...
class Music {
  ...
  async play() {
    ...
    let dispatcher = connection.play(
      ytdl(this.urls[0], { filter: "audioonly" })
    );
  ...
  skip(m) {
    ...
    if (this.urls[1]) {
      ...
      this.leave(
        (m = "Skipped!! Queue is empty. I have left the voice channel!")
      );
    } else if (!this.loop) {
      this.leave(
        (m = "Skipped!! Queue is empty. I have left the voice channel!")
  ...
  async queue() {
    ...
    try {
      this.message.channel
        .send(this.getQueuePages(currentPage, noOfContent))
        .then(async (msg) => {
          msg.react("⏮");
          msg.react("⏭️");
          ...
          const filter = (reaction, user) =>
            (reaction.emoji.name === "⏭️" ||
              ...
              reaction.emoji.name === "▶️" ||
              reaction.emoji.name === "⏮") &&
            this.message.guild.member(user).voice.channel ===
              this.message.guild.member(this.client.user).voice.channel &&
          ...
          collector.on("collect", (r) => {
            switch (r.emoji.name) {
              case "⏭️":
                // Next page
                var totalPage = Math.floor(
                  this.musicHolds.length / noOfContent
                );
              ...
              case "⏮":
                ...
                if (currentPage === 1) {
                  this.message.channel
                    .send("Cannot go to previous page")
                    .then((m) => {
                      setTimeout(function () {
                        m.delete();
                      }, 5000);
  ...
  getQueuePages(page, noOfContent) {
    ...
    for (var i = start; i < end; i++) {
      temptext += `[${i + 1}] ${this.musicHolds[i].title} | | Added By ${
        this.musicHolds[i].requestedBy.username
      } \n \n`;
    }
  ...
  async leave(m) {
    ...
    if (!this.musicHolds[0]) {
      ...
      this.message.channel.send(
        m || "Queue is empty. Left the voice channel!!"
      );
      return;
    } else {
      this.vc.leave();
